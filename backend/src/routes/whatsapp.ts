...
router.get("/qr", async (_req, res: Response) => {
  try {
    ...
    // Use local qrcode-terminal to print in PM2 logs, and render raw QR data
    const html = `<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>WhatsApp QR</title><style>body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;text-align:center;padding:20px;background:#f5f5f5}h2{color:#1a1a1a;margin-bottom:4px}.qr-text{background:#fff;border:2px dashed #ddd;border-radius:12px;padding:16px;margin:16px auto;max-width:90vw;word-break:break-all;font-family:monospace;font-size:11px;color:#555;line-height:1.5;text-align:left}.note{color:#666;margin-top:16px;font-size:14px}.btn{display:inline-block;margin-top:12px;padding:10px 24px;background:#0d6e4e;color:white;border-radius:8px;text-decoration:none;font-weight:600}.small{color:#999;font-size:12px;margin-top:8px}</style></head><body><h2>Scan WhatsApp QR Code</h2><p class="note">To link your WhatsApp:</p><p>1. Open WhatsApp on your phone</p><p>2. Settings → Linked Devices → Link a Device</p><p>3. Point camera at the QR below</p><div id="qrcode" style="display:inline-block;background:white;padding:16px;border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,.08);margin:12px auto"></div><p class="small">QR data: ${qr.substring(0, 30)}... (${qr.length} chars)</p><script src="https://cdn.jsdelivr.net/npm/qrcodejs@1.0.0/qrcode.min.js"></script><script>new QRCode(document.getElementById("qrcode"),{text:${JSON.stringify(qr)},width:256,height:256,correctLevel:QRCode.CorrectLevel.L});</script></body></html>`;
    res.send(html);
  } catch {
