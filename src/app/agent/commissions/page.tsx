...
interface Commission { id: string; dealTitle: string; dealType: string; totalAmount: number; agentCut: number; createdAt: string; }
...
export default function AgentCommissionsPage() {
  ...
  const total = commissions.reduce((s, c) => s + c.agentCut, 0);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="text-xl font-bold text-gray-900">Commissions</h1><p className="text-xs text-gray-500">Your earnings</p></div>
      </div>
            ...
            <tbody>
              {commissions.map(c => (
                <tr key={c.id} className="border-b border-gray-50">
                  ...
                  <td className="px-4 py-3 text-xs font-medium text-emerald-600">₦{c.agentCut.toLocaleString()}</td>
                </tr>
              ))}
