...
export default function AgentCommissions() {
 ...
 const totalEarned = myCommissions.reduce((s, c) => s + c.agentCut, 0);

 return (
 <div className="space-y-6-up">
 <div className="flex items-center gap-3">
 <a href="/agent" className="text-gray-400 hover:text-[var(--color-primary)] transition-colors">
 ...
 <p className="text-sm text-gray-500 mt-0.5">Your earnings from completed deals</p>
 </div>
 </div>
 ...
 <p className="text-xs text-gray-400 mt-1">{myCommissions.length} deal{myCommissions.length !== 1 ? "s" : ""} closed</p>
 </div>
 ...
 <p className="text-sm text-gray-400 mt-1">Commissions appear here when you close deals</p>
 </div>
 ) : (
 ...
 <td className="px-4 py-3.5 text-sm text-right text-emerald-600 font-semibold">{formatNaira(c.agentCut)}</td>
