  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
        ...
        <p className="text-xs text-gray-400 mt-0.5">{myCommissions.length} deal{myCommissions.length !== 1 ? "s" : ""} closed</p>
      </div>
            ...
            <tbody>
              {myCommissions.map((c) => (
                <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-500">{c.dealType}</td>
                  <td className="px-4 py-3 text-sm text-right text-gray-900">{formatNaira(c.totalAmount)}</td>
                  <td className="px-4 py-3 text-sm text-right text-emerald-600 font-medium">{formatNaira(c.agentCut)}</td>
