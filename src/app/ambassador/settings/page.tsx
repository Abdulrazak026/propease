...
export default function AmbassadorSettings() {
 ...
 <input defaultValue={currentUser?.name || ""} className="w-full rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/20" />
 </div>
 <div className="col-span-2">
 <label className="block text-xs font-medium text-gray-700 mb-1">City Description</label>
 <textarea defaultValue={`Manage real estate listings for ${currentUser?.city || "your city"}.`} rows={3} className="w-full rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/20 resize-none" />
 </div>
 </div>
 ...
 <div key={item.key} className="flex items-center justify-between py-3 px-4 rounded-lg bg-gray-50/50 border border-gray-100">
