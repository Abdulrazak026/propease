      ...
      <div className="max-w-[1400px] w-full mx-auto px-5 sm:px-6 lg:px-10 py-6">
        <div className="flex items-center justify-between gap-3 mb-5">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {loading ? "Loading…" : `${listings.length} ${listings.length === 1 ? "property" : "properties"}`}
            </p>
          </div>
        ...
        {view === "map" ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
            <p className="text-sm font-semibold text-gray-900 mb-1">Nothing matches</p>
            <p className="text-xs text-gray-500">Try changing your filters.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
