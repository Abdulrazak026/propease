          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {[
                ...
                <li key={link.h}><Link href={link.h} className="text-sm text-gray-500 hover:text-gray-900 min-h-[44px] inline-flex items-center">{link.l}</Link></li>
              ))}
            </ul>
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 pt-6 border-t border-gray-100">
          {socialItems.length > 0 && (
            <div className="flex items-center gap-3">
              ...
              {socialItems.map(s => (
                <a key={s.key} href={g(s.key)} target="_blank" rel="noopener noreferrer" className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-gray-500 hover:bg-[var(--color-primary)] hover:text-white transition-colors active:scale-90" aria-label={s.label}>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <input type="email" placeholder="Enter your email" className="text-sm border border-gray-200 rounded-lg px-3 py-2.5 flex-1 sm:w-48 focus:outline-none focus:ring-1 focus:ring-[var(--color-primary)]" />
            <button className="text-sm bg-[var(--color-primary)] text-white px-4 py-2.5 rounded-lg hover:opacity-90 font-medium min-h-[44px]">Subscribe</button>
          </div>
        </div>
