...
export default function Navbar() {
  ...
  return (
    <>
    {/* Mobile header */}
    <header className="md:hidden w-full bg-white border-b border-gray-200">
    ...
    {/* Desktop header */}
    <header className="hidden md:flex w-full bg-white border-b border-gray-200">
          ...
          {mounted && isAuthenticated && currentUser && (
            <div ref={userRef} className="relative">
              <button onClick={() => setUserOpen(!userOpen)} className="flex items-center gap-2.5 pl-1 pr-3 py-1 rounded-full hover:bg-gray-50 transition-colors">
                ...
                <span className="text-sm text-gray-700 font-medium capitalize hidden md:inline">{currentUser.role === "head" ? "admin" : currentUser.role}</span>
