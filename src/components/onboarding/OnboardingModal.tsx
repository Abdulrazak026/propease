...
export default function OnboardingModal({ role }: OnboardingModalProps) {
  ...
  useEffect(() => {
    if (!role) return;
    const seen = localStorage.getItem(`propease-onboarding-${role}`);
    if (!seen) {
      setVisible(true);
    }
  }, [role]);
  ...
  const dismiss = () => {
    if (role) localStorage.setItem(`propease-onboarding-${role}`, "1");
    setVisible(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={dismiss}>
      <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4 shadow-xl animate-fade-in-up" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-gray-50 rounded-xl p-5 border border-gray-100 mb-5 min-h-[120px] flex flex-col justify-center">
